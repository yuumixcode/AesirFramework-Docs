...
版本检测按「直连 GitHub → 镜像站 → CDN 中转」三层兜底,**能直连 GitHub 就是 100% 最新**(窗口会显示本次直连是否可用)。只有当直连与镜像站都不可用、结果来自 CDN 中转时,才可能因分支缓存(最长约 12 小时)拿到旧版本 —— 此时窗口会给出延迟提示,可点「打开 Releases 页面」人工确认。
