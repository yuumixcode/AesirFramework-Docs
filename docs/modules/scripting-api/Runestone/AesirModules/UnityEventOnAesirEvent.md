---
title: UnityEventOnAesirEvent
description: "Runestone.AesirModules.UnityEventOnAesirEvent 的 API 文档"
---

# `UnityEventOnAesirEvent`
...
**继承链:** `System.Object` → `UnityEngine.Object` → `UnityEngine.Component` → `UnityEngine.Behaviour` → `UnityEngine.MonoBehaviour` → `UnityEventOnAesirEvent`
...
public class UnityEventOnAesirEvent : UnityEngine.MonoBehaviour
```

UnityEvent 桥接组件。监听指定类型的 AesirEventArgs， 事件触发时调用 Inspector 中配置的 UnityEvent 回调， 供非程序员在 Inspector 中串联事件响应，无需编写订阅代码。
用法：挂载本组件 → 经 SubclassSelectorAttribute 下拉选择监听的事件类型 → 在 On Raised 中绑定任意 UnityEvent 回调（播放音效、激活物体等）。
...
| 名称 | 描述 |
| :--- | :--- |
| [`UnityEventOnAesirEvent()`](#constructor-unityeventonaesirevent) | — |
