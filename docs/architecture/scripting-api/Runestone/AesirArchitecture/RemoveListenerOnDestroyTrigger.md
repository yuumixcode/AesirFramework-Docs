---
title: RemoveListenerOnDestroyTrigger
description: "Runestone.AesirArchitecture.RemoveListenerOnDestroyTrigger 的 API 文档"
---

# `RemoveListenerOnDestroyTrigger`
...
**继承链:** `System.Object` → `UnityEngine.Object` → `UnityEngine.Component` → `UnityEngine.Behaviour` → `UnityEngine.MonoBehaviour` → `Runestone.AesirArchitecture.RemoveListenerTrigger` → `RemoveListenerOnDestroyTrigger`

## 声明

``` csharp
[DisallowMultipleComponent]
public sealed class RemoveListenerOnDestroyTrigger : Runestone.AesirArchitecture.RemoveListenerTrigger
```
...
[DisallowMultipleComponent] 防止在同一 GameObject 上重复挂载本组件。
两种使用方式：
推荐：编辑阶段提前挂载——订阅时框架经 GetComponent 直接复用， 避免运行时 AddComponent 的开销。在 Inspector 中手动添加本组件属正常且推荐的做法。 兜底：运行时自动挂载——未预挂载时， RemoveListenerExtensions.RemoveListenerWhenGameObjectOnDestroyed 扩展方法 会在订阅时自动为本 GameObject 添加本组件（方便性兜底，付一次 AddComponent 开销）。
两种方式经 [DisallowMultipleComponent] 保证不会重复添加；在 Inspector 中看到本组件属于正常现象。
...
| 名称 | 描述 |
| :--- | :--- |
| [`RemoveListenerOnDestroyTrigger()`](#constructor-removelistenerondestroytrigger) | — |
...
``` csharp
public RemoveListenerOnDestroyTrigger()
```
