---
title: RemoveListenerTrigger
description: "Runestone.AesirArchitecture.RemoveListenerTrigger 的 API 文档"
---

# `RemoveListenerTrigger`
...
**继承链:** `System.Object` → `UnityEngine.Object` → `UnityEngine.Component` → `UnityEngine.Behaviour` → `UnityEngine.MonoBehaviour` → `RemoveListenerTrigger`

## 声明

``` csharp
public abstract class RemoveListenerTrigger : UnityEngine.MonoBehaviour
```

自动移除监听调用器基类。维护监听句柄列表，子类在特定生命周期事件中调用 RemoveAllListeners 批量移除。

**备注**

作为抽象基类，本身不绑定任何 Unity 生命周期回调。子类通过在 OnDestroy、OnDisable 等回调中 调用 RemoveAllListeners 来触发批量清理，从而将"何时移除监听"的策略交由子类决定。
...
| 名称 | 描述 |
| :--- | :--- |
| [`AddRemoveListenerHandle(AutoRemoveListenerHandle)`](#method-addremovelistenerhandle-autoremovelistenerhandle) | 添加监听句柄，使其在调用条件满足时自动移除 |
...
将批量清理委托给 RemoveAllListeners 执行， 确保所有已注册的监听句柄按统一流程被正确移除并释放引用。
