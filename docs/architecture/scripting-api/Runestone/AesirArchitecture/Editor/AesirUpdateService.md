...
更新流程：检测远程版本 → 对比本地版本（直接读取包内 package.json）→ 拉取并展示「本地 → 远程」 更新日志（BuildChangelogDigestAsync）→ 确认框二次确认（ BuildUpdateConfirmation）→ 更新前自动备份（用户可能修改过代码）→ 按"上次安装清单 − 新版清单"精确差集清理残留（不误伤用户新增文件）→ 下载 .unitypackage → 静默导入 → 逐包登记安装清单。
...
| 名称 | 描述 |
| :--- | :--- |
| [`PrimaryInstallRoot`](#property-primaryinstallroot) | 主安装根（项目相对路径）——备份源与提示文案用；委托 PrimaryInstallRoot 动态解析（默认根优先，无任何本地安装时回退 InstallRootRelativePath）。 |
...
主安装根（项目相对路径）——备份源与提示文案用；委托 PrimaryInstallRoot 动态解析（默认根优先，无任何本地安装时回退 InstallRootRelativePath）。
...
| [`BuildChangelogDigestAsync(string, IReadOnlyList<AesirUpdateService.InstalledPackage>)`](#method-buildchangelogdigestasync-string-ireadonlylist-aesirupdateservice-installedpackage) | 生成「本地版本 → 远程版本」的更新日志摘要文本（每包一节，按传入顺序）。 远程优先：按 tag 拉取包内 CHANGELOG.md 并提取比本地新的段落；远程拉取失败时回退本地包内 CHANGELOG 的最新段落并标注来源。日志属辅助信息，任何单包失败不影响其余包与其摘要输出。 |
...
| [`UpdatePackagesAsync(AesirUpdateService.ReleaseSnapshot, IReadOnlyList<AesirUpdateService.InstalledPackage>, Action<string, float>)`](#method-updatepackagesasync-aesirupdateservice-releasesnapshot-ireadonlylist-aesirupdateservice-installedpackage-action-string-float) | 执行更新：整体备份 → 逐包（下载 → 按清单差集清残留 → 静默导入 → 登记安装清单）。 返回备份目录的绝对路径（无安装源时备份为 null）。 |
...
| [`BuildUpdateConfirmation(IReadOnlyList<AesirUpdateService.InstalledPackage>, string, bool)`](#method-buildupdateconfirmation-ireadonlylist-aesirupdateservice-installedpackage-string-bool) | 构建更新前的确认框文本：逐包列示「本地 v旧 → 远程新」，附备份与覆盖说明； isGitRepository 为 true 时追加开发仓库警告。 |
...
### BuildChangelogDigestAsync(string, IReadOnlyList<AesirUpdateService.InstalledPackage>) {#method-buildchangelogdigestasync-string-ireadonlylist-aesirupdateservice-installedpackage}
...
``` csharp
[AsyncStateMachine]
public static async Task<string> BuildChangelogDigestAsync(string remoteTag, IReadOnlyList<AesirUpdateService.InstalledPackage> outdatedPackages)
```
...
### UpdatePackagesAsync(AesirUpdateService.ReleaseSnapshot, IReadOnlyList<AesirUpdateService.InstalledPackage>, Action<string, float>) {#method-updatepackagesasync-aesirupdateservice-releasesnapshot-ireadonlylist-aesirupdateservice-installedpackage-action-string-float}

执行更新：整体备份 → 逐包（下载 → 按清单差集清残留 → 静默导入 → 登记安装清单）。 返回备份目录的绝对路径（无安装源时备份为 null）。

``` csharp
[AsyncStateMachine]
public static async Task<string> UpdatePackagesAsync(AesirUpdateService.ReleaseSnapshot snapshot, IReadOnlyList<AesirUpdateService.InstalledPackage> targets, Action<string, float> onProgress)
```
...
| `targets` | `IReadOnlyList<AesirUpdateService.InstalledPackage>` | 待更新包列表，须已按依赖顺序排列（Architecture 先于 Modules）。 |
...
### BuildUpdateConfirmation(IReadOnlyList<AesirUpdateService.InstalledPackage>, string, bool) {#method-buildupdateconfirmation-ireadonlylist-aesirupdateservice-installedpackage-string-bool}

构建更新前的确认框文本：逐包列示「本地 v旧 → 远程新」，附备份与覆盖说明； isGitRepository 为 true 时追加开发仓库警告。

``` csharp
public static string BuildUpdateConfirmation(IReadOnlyList<AesirUpdateService.InstalledPackage> targets, string remoteVersion, bool isGitRepository)
```
...
| 名称 | 类型 | 说明 |
| :--- | :--- | :--- |
| `targets` | `IReadOnlyList<AesirUpdateService.InstalledPackage>` | — |
| `remoteVersion` | `string` | — |
